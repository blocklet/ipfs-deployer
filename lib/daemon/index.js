...
const daemonLog = {
  info: debug('ipfs-deployer:daemon:stdout'),
  err: debug('ipfs-deployer:daemon:stderr'),
};
...
class Daemon {
      ...
      if (this.disposable) {
        ...
        if (this.opts.forceKill !== false) {
          killTimeout = setTimeout(() => {
            ...
            console.error(
              new Error(
                `Timeout stopping ${this.opts.type} node after ${this.opts.forceKillTimeout}ms. Process ${this.subprocess.pid} will be force killed now.`
              )
            );
            this.subprocess.kill('SIGKILL');
  ...
  async _getConfig(key = 'show') {
    const { stdout } = await execa(this.exec, ['config', key], {
      env: this.env,
    }).catch(translateError);
  ...
  async _replaceConfig(config) {
    ...
    await execa(this.exec, ['config', 'replace', `${tmpFile}`], { env: this.env }).catch(translateError);
